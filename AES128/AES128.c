// aes128_pure_c.c
// AES-128 Encryption - Pure C Implementation

#include <stdio.h>
#include <stdint.h>
#include <string.h>

// AES S-box
static const uint8_t sbox[256] = {
    
};

// Rcon values for key expansion
static const uint8_t Rcon[11] = {
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

// Galois Field multiplication by 2
uint8_t gmul2(uint8_t a) {
    return (a << 1) ^ (((a >> 7) & 1) * 0x1b);
}

// Galois Field multiplication by 3
uint8_t gmul3(uint8_t a) {
    return gmul2(a) ^ a;
}

// SubBytes transformation
void SubBytes(uint8_t state[16]) {
    for (int i = 0; i < 16; i++) {
        state[i] = sbox[state[i]];
    }
}

// ShiftRows transformation
void ShiftRows(uint8_t state[16]) {
    uint8_t temp;

    // Row 1: shift left by 1
    temp = state[1];
    state[1] = state[5];
    state[5] = state[9];
    state[9] = state[13];
    state[13] = temp;

    // Row 2: shift left by 2
    temp = state[2];
    state[2] = state[10];
    state[10] = temp;
    temp = state[6];
    state[6] = state[14];
    state[14] = temp;

    // Row 3: shift left by 3
    temp = state[15];
    state[15] = state[11];
    state[11] = state[7];
    state[7] = state[3];
    state[3] = temp;
}

// MixColumns transformation
void MixColumns(uint8_t state[16]) {
    uint8_t temp[16];

    for (int i = 0; i < 4; i++) {
        int idx = i * 4;
        temp[idx] = gmul2(state[idx]) ^ gmul3(state[idx + 1]) ^ state[idx + 2] ^ state[idx + 3];
        temp[idx + 1] = state[idx] ^ gmul2(state[idx + 1]) ^ gmul3(state[idx + 2]) ^ state[idx + 3];
        temp[idx + 2] = state[idx] ^ state[idx + 1] ^ gmul2(state[idx + 2]) ^ gmul3(state[idx + 3]);
        temp[idx + 3] = gmul3(state[idx]) ^ state[idx + 1] ^ state[idx + 2] ^ gmul2(state[idx + 3]);
    }

    memcpy(state, temp, 16);
}

// AddRoundKey transformation
void AddRoundKey(uint8_t state[16], uint8_t roundKey[16]) {
    for (int i = 0; i < 16; i++) {
        state[i] ^= roundKey[i];
    }
}

// Key Expansion
void KeyExpansion(uint8_t key[16], uint8_t roundKeys[176]) {
    memcpy(roundKeys, key, 16);

    for (int i = 4; i < 44; i++) {
        uint8_t temp[4];
        memcpy(temp, &roundKeys[(i - 1) * 4], 4);

        if (i % 4 == 0) {
            // RotWord
            uint8_t k = temp[0];
            temp[0] = temp[1];
            temp[1] = temp[2];
            temp[2] = temp[3];
            temp[3] = k;

            // SubWord
            temp[0] = sbox[temp[0]];
            temp[1] = sbox[temp[1]];
            temp[2] = sbox[temp[2]];
            temp[3] = sbox[temp[3]];

            // Rcon
            temp[0] ^= Rcon[i / 4];
        }

        for (int j = 0; j < 4; j++) {
            roundKeys[i * 4 + j] = roundKeys[(i - 4) * 4 + j] ^ temp[j];
        }
    }
}

// AES-128 Encryption
void AES128_Encrypt(uint8_t plaintext[16], uint8_t key[16], uint8_t ciphertext[16]) {
    uint8_t state[16];
    uint8_t roundKeys[176];

    memcpy(state, plaintext, 16);
    KeyExpansion(key, roundKeys);

    // Initial round
    AddRoundKey(state, &roundKeys[0]);

    // Main rounds (9 rounds)
    for (int round = 1; round < 10; round++) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, &roundKeys[round * 16]);
    }

    // Final round (no MixColumns)
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, &roundKeys[10 * 16]);

    memcpy(ciphertext, state, 16);
}

// Print hex
void print_hex(const char* label, uint8_t data[16]) {
    printf("%s: ", label);
    for (int i = 0; i < 16; i++) {
        printf("%02x ", data[i]);
    }
    printf("\n");
}

// Main test function
int main() {
    // Test vector from NIST FIPS-197
    uint8_t plaintext[16] = {
        0x32, 0x43, 0xf6, 0xa8, 0x88, 0x5a, 0x30, 0x8d,
        0x31, 0x31, 0x98, 0xa2, 0xe0, 0x37, 0x07, 0x34
    };

    uint8_t key[16] = {
        0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6,
        0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c
    };

    uint8_t ciphertext[16];

    printf("=== AES-128 Encryption Test (Pure C) ===\n\n");

    print_hex("Plaintext ", plaintext);
    print_hex("Key       ", key);

    AES128_Encrypt(plaintext, key, ciphertext);

    print_hex("Ciphertext", ciphertext);

    // Expected result for verification
    uint8_t expected[16] = {
        0x39, 0x25, 0x84, 0x1d, 0x02, 0xdc, 0x09, 0xfb,
        0xdc, 0x11, 0x85, 0x97, 0x19, 0x6a, 0x0b, 0x32
    };

    print_hex("Expected  ", expected);

    // Verify
    int match = 1;
    for (int i = 0; i < 16; i++) {
        if (ciphertext[i] != expected[i]) {
            match = 0;
            break;
        }
    }

    printf("\nResult: %s\n", match ? "PASS ✓" : "FAIL ✗");

    return 0;
}